...
if nargin == 1
    n = prod(size(m)); %#ok<PSIZE> .. for now.
    % TODOD A better method might be builtin('numel', M) where M is a
    % non-empty element of m.multivector, but finding a non-empty element
    % is costly. See size.m for a way of doing this.
else
...
% $Id: numel.m 271 2021-07-11 19:54:47Z sangwine $
