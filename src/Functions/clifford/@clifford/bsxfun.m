...
% depend on bsxfun. This code was developed as part of the Quaternion
% Toolbox for Matlab (QTFM) with only trivial modifications here for the
% Clifford multivector case.
...
% the algorithm implemented here will work for any meaningful function that
% obeys the rules for bsxfun (see the documentation on the Matlab version
% of bsxfun for these rules).

% We don't check the types of the A and B parameters either. They could
% both be multivectors, or one could be numeric/logical. The function
...
% this code overcomplicated. Similarly for multivectors with different
...
% Since this is a class method, normal array indexing does not work and we
% have to use subsref/subasgn.
...
% $Id: bsxfun.m 271 2021-07-11 19:54:47Z sangwine $
