...
for j = 1:clifford_descriptor.m    
    if isempty(get(a, j)) && isempty(get(b, j))
    ...
    % supply zeros and not empty. We call part, because it supplies the
    % zeros if needed. (There is some overhead in this because of the error
    % checking.)
    
    c.multivector{j} = horzcat(part(a, j), part(b, j));
end
...
% $Id: horzcat.m 271 2021-07-11 19:54:47Z sangwine $
