...
<!-- This XML stylesheet converts an XML description of a Clifford function
     into an HTML documentation page for display in the Matlab help
     browser. See the file readme.txt for further details. Copied from the
...
<xsl:comment> Generated automatically from an XML file of the same name.
     Copyright: Stephen J. Sangwine and Eckhard Hitzer, 2011-2021.
</xsl:comment>
<html>
<head>
<title>
<xsl:value-of select="@name"/> :: Functions (Clifford Multivector Toolbox Function Reference)
</title>
<link rel="stylesheet" href="clifstyle.css" type="text/css"></link>
</head>
<body>
<h1>Clifford Function Reference</h1>
<h2><xsl:value-of select="@name"/></h2>
<xsl:apply-templates/>
<h4>(c) 2011-2021 Stephen J. Sangwine and Eckhard Hitzer.</h4>
...
<br/>(Clifford overloading of standard &matlab; function)
</xsl:if>
</p>
</xsl:template>
...
<h2>See Also</h2>
<!-- Here we output a list of comma separated hyperlinks, one for Clifford functions and one for Matlab
     functions. What we output differs according to whether there are 0, 1 or more functions to be
     listed. -->
<xsl:if test="count(cliffun)=1">Clifford function: <xsl:apply-templates select="cliffun"/><br/></xsl:if>
<xsl:if test="count(matlabfun)=1">&matlab; function: <xsl:apply-templates select="matlabfun"/><br/></xsl:if>
<xsl:if test="count(cliffun)>1">Clifford functions: <xsl:apply-templates select="cliffun"/><br/></xsl:if>
...
<!--$Id: cliffunction.xsl 262 2021-06-16 16:39:12Z sangwine $-->
