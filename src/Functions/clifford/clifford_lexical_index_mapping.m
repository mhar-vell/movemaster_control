...
% Copyright © 2011, 2016, 2019 Stephen J. Sangwine and Eckhard Hitzer.
...
%    HOWEVER, the values are stored with 0-based indexing in order to allow
%    a maximum value of 65536 (which is stored as 65535) in a uint16 (this
%    is necessary to economise on memory/storage when n=16). (This was
%    changed with version 1.4 in April 2019 to fix a latent bug.)
%
% 2. Binary index. An n-bit word (conceptually) contains one bit
%    representing each basis vector (e1, e2, e3 etc.). A combination of
...
%
% T an array of uint16 index values.
% R an array of uint16 reverse index values.
...
assert(n <= 16,      'Parameter must be 16 or less.');

m = 2.^uint32(n); % Make sure this is uint32 so that m can be 65536, max.
...
K = 0:m - 1; %#ok<BDSCI>  This generates the set of index
                        % values 0, 1, 2, ... 65535.
L = cell(1, m); % This is used for the strings 'e1', 'e12', ... etc.
G = zeros(size(K), 'uint8'); % To store the grade for each value of K.

for I = 1:length(K)
    L{I} = clifford_literal(K(I), n);
...
[~, I] = sort(T); % This returns a double array of index values.

R = uint16(I - 1); % Convert the array of index values to reduce memory usage.

end

% $Id: clifford_lexical_index_mapping.m 270 2021-07-11 19:42:06Z sangwine $
