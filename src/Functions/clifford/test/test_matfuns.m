...
tdisp('Testing Matlab functions that work with multivectors ...')
...
% flip   - Flip matrix along specified dimension
% fliplr - Flip matrix in left/right direction
% flipud - Flip matrix in up/down direction

A = randi(100, 5, 4);

check(all(all(flip(A, 1) == part(flip(clifford(A), 1), 1))), 'flip fails test 1')
check(all(all(flip(A, 2) == part(flip(clifford(A), 2), 1))), 'flip fails test 2')

check(all(all(fliplr(A) == part(fliplr(clifford(A)), 1))), 'fliplr fails test')
check(all(all(flipud(A) == part(flipud(clifford(A)), 1))), 'flipud fails test')
...
% $Id: test_matfuns.m 244 2020-05-28 20:53:28Z sangwine $
